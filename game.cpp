...
void load_profile_names();
void save_profile_names();
...
while(1)
	{
		...
		start_menu();
			...
			{	
				...
				cprintf("\b");
				int choice;
				if(plyr[x].balance<0&&plyr[x].bankrupted!='y')
				bankrupt(&plyr[x]);
		...
		case 1:				
				...
				for(int i=0;i<3;++i)
					...
					fout<<bank.balance<<'\n'<<bank.loan<<'\n';
						...
						getch();
						break;
		case 2:remove(Save[choice-1]);
				gotoxy(68,38);
						textcolor(15);
						cprintf("Deleted successfully...");
						Save[choice-1][0]='\0';
...
void load()
{
	...
	choice=1;
		...
		ifstream f_in;
		f_in.open(Save[choice-1],ios::in);
